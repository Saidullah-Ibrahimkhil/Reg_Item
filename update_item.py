...
class UpdateItem(tb.Toplevel):
    # Show registeration form and add items to data base via model
    
    def __init__(self, item_data, callback,*args, **kwargs):
    ...
    def create_widget(self):
        ...
        self.device_name.insert('', self.item_data[1])
        ...
        self.device_serial.insert('', self.item_data[2])
        #label 
        ...
        self.device_location.insert('', self.item_data[3])
        self.submit_date = tb.DateEntry(self,firstweekday=5)
        self.submit_date.grid(row=3, column=1, sticky='w', padx=10, pady=(0,10))
        submit_date = self.item_data[6].split()
        submit_date = submit_date[0].split('-')
        submit_date = submit_date[1]+'/'+submit_date[2]+'/'+submit_date[0]
        self.submit_date.entry.delete(0,END)
        self.submit_date.entry.insert(0,submit_date)
        #label 
        ...
        self.deliverer_name.insert('', self.item_data[7])
        self.delivery_date = tb.DateEntry(self,firstweekday=5)
        self.delivery_date.grid(row=7, column=1, sticky='w', padx=10, pady=(0,10))
        if self.item_data[8] is not None:
            delivery_date = self.item_data[8].split()
            delivery_date = delivery_date[0].split('-')
            delivery_date = delivery_date[1]+'/'+delivery_date[2]+'/'+delivery_date[0]

        else: delivery_date = ''
        self.delivery_date.entry.delete(0,END)
        self.delivery_date.entry.insert(0,delivery_date)
        # buttons
        buttons_frame = tb.Frame(self)
        buttons_frame.grid(row=9, column=1, sticky='e', pady=(15,0), padx=20)
        save_button = tb.Button(buttons_frame, text='Save', bootstyle = 'info', command=self.update_item)
    ...
    def update_item(self):
        ...
        if valid:
            item_data = {
                            ...
                            'submitter_person':self.submitter_name.get(),
                            'submitter_contact':self. submitter_contact.get(),
                            'submit_date':self.valid_date(self.submit_date.entry.get()),
                            'deliverer_person':self.deliverer_name.get(),
                            'delivery_date':self.valid_date(self.delivery_date.entry.get())
                        }
            my_item =  Item()
            f_result = my_item.update(update_data=item_data)
            self.feedback(f_result)
            self.callback()
        else:
            invalid_frame = tb.Frame(self)
            invalid_frame.grid(row=8, column=0, columnspan=2)
            if len(invalid_input[0])  !=0:
                tb.Label(invalid_frame, text=f'Missing Inputs: '+self.show_error(invalid_input[0]),font=('Trebuchet MS', 8), bootstyle='danger').grid(row=0, column=0, sticky='w', padx=10)
            if len(invalid_input[1]) !=0:
                tb.Label(invalid_frame, text=f'Invalid Inputs: '+self.show_error(invalid_input[1]),font=('Trebuchet MS', 8), bootstyle='danger').grid(row=1, column=0, sticky='w', padx=10)
    ...
    def feedback(self, feedback_result):
        ...
        feedback = Feedback(feedback=add_feedback, operation='update')
